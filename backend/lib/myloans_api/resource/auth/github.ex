defmodule MyloansApi.Resource.Auth.Github do
  def authenticate(code) do
    fetch_github_token(code)
    |> fetch_github_userdata()
    |> fetch_koha_userdata()
    |> create_session()
  end

  defp fetch_github_token(code) do
    token_body = %{
      code: code,
      grant_type: "authorization_code",
      redirect_uri: System.get_env("OAUTH2_REDIRECT_URI") || "http://localhost:8080/login/github"
    }
    ...
    case URI.decode_query(resp.body) do
  ...
  defp fetch_github_userdata({:error, status}), do: {:error, status}
  defp fetch_github_userdata({:ok, :token, github_token}) do
    user_endpoint = System.get_env("OAUTH2_USER_ENDPOINT")
    userdata_resp = HTTPoison.get!(user_endpoint, [{"Accept", "application/json"}, {"Authorization", "Bearer " <> github_token}])
    case Jason.decode!(userdata_resp.body) do
      %{"login" => login} -> {:ok, :login_success, login}
      _ -> {:error, :github_userdata_fail}
    end
  end
